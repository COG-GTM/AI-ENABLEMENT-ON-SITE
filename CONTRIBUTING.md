...
2. Make the change. New procedures go in `.devin/skills/<name>/SKILL.md` and get one row in `AGENTS.md`; new numbers come from a tool, not from typing.
   - Frontmatter keys allowed: `name`, `description`, `argument-hint`, `model`, `allowed-tools`, `permissions`, `triggers`. Cap is 15 skills (`MAX_SKILLS` in `tools/check_repo.py`); merge before adding a sixteenth.
