...
| SN-REQ-007 | A temperature reading outside -40 to +85 C shall set the FAULT flag and keep the last valid value. | Must | `test_fault`, SN-HAZ-002 |
| SN-REQ-008 | If the IMU fails to respond for 5 consecutive samples, the node shall set the FAULT flag and reinitialise the IMU. | Must | `test_fault`, SN-HAZ-001 |
