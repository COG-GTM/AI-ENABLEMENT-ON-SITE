    ...
    def test_local_override_and_user_file_are_validated_too(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            try:
                ...
                # Model B: a personal-secret override must repeat command/args (merge semantics are undocumented)
                full = '{"mcpServers": {"reference-system": {"command": "python3", "args": ["integrations/reference-mcp/server.py"], "env": {"JIRA_TOKEN": "${env:JIRA_TOKEN}"}}}}'
                local.write_text(full)
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "OK", r)
                ...
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("mcp_config.local.json: reference-system overrides .devin/mcp_config.json but omits args, command", r["detail"])
                # command present but args dropped: a replacing build would launch the wrong thing
                local.write_text('{"mcpServers": {"reference-system": {"command": "python3", "env": {"JIRA_TOKEN": "${env:JIRA_TOKEN}"}}}}')
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("reference-system overrides .devin/mcp_config.json but omits args", r["detail"])
                # switching kind (command -> url) is rejected for the same reason; a merging build would see both
                local.write_text('{"mcpServers": {"reference-system": {"url": "https://h/mcp"}}}')
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("reference-system overrides .devin/mcp_config.json but omits args, command", r["detail"])
                # remote override that drops transport/headers
                user.write_text('{"mcpServers": {"team": {"url": "https://h/mcp", "transport": "sse", "headers": {"Authorization": "Bearer ${env:T}"}}}}')
                local.write_text('{"mcpServers": {"team": {"url": "https://h/mcp"}}}')
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("mcp_config.local.json: team overrides user mcp_config.json but omits headers, transport", r["detail"])
                local.write_text('{"mcpServers": {"team": {"url": "https://h/mcp", "transport": "http", "headers": {"Authorization": "Bearer ${env:T}"}, "disabled": true}}}')
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "OK", r)
                self.assertIn("2 server(s)", r["detail"])
                # keeping the key with a null value does not turn a remote entry into a local one
                local.write_text('{"mcpServers": {"team": {"url": null, "transport": "http", "headers": {}, "command": "python3"}}}')
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("mcp_config.local.json: team: use either url (remote) or command (local), not both", r["detail"])
                local.write_text('{"mcpServers": {"team": {"url": null, "transport": "http", "headers": {}}}}')
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("mcp_config.local.json: team: url must start with https://", r["detail"])
                user.unlink()
                local.write_text(full.replace("${env:JIRA_TOKEN}", "hunter2hunter2"))
                r = doctor.check_mcp_config()
                self.assertEqual(r["status"], "FAIL")
                self.assertIn("mcp_config.local.json: reference-system: env.JIRA_TOKEN looks like a literal secret", r["detail"])
