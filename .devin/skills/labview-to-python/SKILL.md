...
hand in the shape of a LabVIEW HTML export (the images are replaced by text). TestStand `.seq` and TDMS are
accepted inputs with no example file here. Every number in the recording is synthetic. What the example
proves is the method: the replay, the compare, the FAIL row that has to survive, and the review that names
what is still unproven.

## Worked example 2: real `.vi` files, bare-binary input

`example-system/real-vi/` is the same lane run on four real VIs from a public open-source LabVIEW project
(MQTT topic-filter validation and matching, plus the project's own two requirement-test VIs), with only the
binaries as input. Read `VI-REVIEW.md` there before promising anything about a customer's `.vi`:

- `lvkit describe` and `render` worked on 4 of 4; `lvkit unresolved` named the primitives and SubVIs it
  could not map; `lvkit generate` **failed on 4 of 4**. The Python was written from the inventory
  (`VI-INVENTORY.md`, generated by `inventory.py --write`, which also records the `unresolved` and `generate`
  output per file) and the rendered diagram.
- Expected outputs came from two places that are not the code: the input strings and verdicts read out of the
  project's own test VIs, and the public MQTT 3.1.1 specification. `cases.csv` keeps the diagram reading and
  the specification in separate columns; `topic_filter.py --spec-diff` lists where they disagree.
- `bench_compare` PASS means the port agrees with the diagram reading on every row. It does **not** mean
  the port equals the VI: nobody ran the VI. The review lists each reading that one recorded run would
  settle. Say "specification-level reconstruction", not "equivalent", until that run exists.
- The port surfaced a real finding (the project swaps `+` and `#` relative to the specification, in code
  and tests alike). Report it; do not "fix" it in the port. The port reproduces the VI.
